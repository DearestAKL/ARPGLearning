...
namespace TreeDesigner
{
    ...
    public abstract partial class BaseNode
    {
        ...
        public virtual void BeforeInit()
        {
            ...
            foreach (var fieldInfo in this.GetAllFields())
            {
                if (fieldInfo.GetValue(this) is PropertyPort propertyPort)
                {
                    ...
                    {
                        //Shared.Schedule.AddFrameSchedule(this, "delay", 1,() =>
                        //{
                        //    Debug.Log($"{m_Owner.name}:{this.GetAttribute<NodeNameAttribute>().Name} already contains {propertyPort.Name}");
                        //});
                    }
                    else
        ...
        public virtual int GetPbTypeId()
        {
            return 0;
        }
        ...
        public bool HasPropertyEdge()
        {
            for (int i = 0; i < m_Owner.PropertyEdges.Count; i++)
            {
                if (m_Owner.PropertyEdges[i].StartNode is ExposedPropertyNode exposedPropertyNode && m_Owner.PropertyEdges[i].EndNode == this)
                {
                    return true;
                }
            }
